...
static gboolean capture_done_idle(gpointer data)
{
    ...
    if (res->ok) {
        ...
        update_status_labels(app, &res->status);
        append_log(app, "frame %u captured from buffer %u: %u bytes, %.2f ms, %.2f fps",
                   res->frame.frame_counter, res->frame.buffer_index, res->frame.valid_size,
...
static void on_connect_clicked(GtkButton *button, gpointer data)
{
    ...
    append_log(app, "connected: %ux%u RGB565 frame=%u buffers=%u each=%u", info.width, info.height,
               info.frame_size, info.buffer_count, info.buffer_size);
    if (query_status(app, &st))
        update_status_labels(app, &st);
    set_buttons_idle(app);
}
